...
- Approval normally requires at least two source families. If any official candidate is available, the approved set must include an official candidate unless the Director explicitly audits an official-exclusion boolean and non-empty reason. A single-family approval requires an explicit boolean fallback and non-empty reason, both persisted in the approval audit.
