...
6. 使用 `character_submit_proposal` 提交，攜帶目前 task、lease、base workflow revision 與 artifact CAS；不得直接寫作者檔或指定正式路徑。若工具回報參數格式錯誤，依工具 input schema 修正參數後重試同一工具；禁止改用 `task_submit`、偽造 artifact reference 或把尚未套用的 Proposal 宣告為完成。
   - 若 Director 已代為 claim（`workflow_status` 顯示 task 為 `claimed` 且 `lease.owner` 是其他 Agent、`resumable_tasks` 內含該 task 與既有 `lease.id`），直接以既有 lease 使用 `task_context` 與 `character_submit_proposal`，**不重複 claim**。
   - 多 Creator 並行時提交是短交易：若提交被 `WORKFLOW_REVISION_CONFLICT` 或 `TRANSACTION_LOCKED` 拒絕，這不是內容錯誤 — 呼叫 `workflow_status` 重讀最新 `workflow.revision` 與作者檔 source revision，以新 revision 重新提交同一份 proposal，可重試至成功；禁止因此標記 task 失敗。
