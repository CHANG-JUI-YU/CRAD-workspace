...
export function registerPlannerRoutes(app: FastifyInstance, context: DashboardContext): void {
  ...
  app.post("/api/planner/simulate", async (request) => {
    const input = z.object({ project_id: stableIdSchema, token_budget: z.number().int().positive().optional(), conversation: z.array(z.string()).default([]), strict: z.boolean().default(false) }).strict().parse(request.body);
    const built = await buildProject({ workspaceRoot: context.workspaceRoot, projectId: projectId(input.project_id), publish: false, png: false, strict: input.strict, ...(input.token_budget !== undefined ? { tokenBudget: input.token_budget } : {}) });
    return { ok: true, data: { token: built.tokenReport, trigger: simulateTriggers(built.planned, { messages: input.conversation, budgetIncludedEntryIds: built.tokenReport.entries.filter((entry) => entry.included).map((entry) => entry.entry_id) }), plan: built.planned } };
  });
}
