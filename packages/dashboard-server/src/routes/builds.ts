...
export function registerBuildRoutes(app: FastifyInstance, context: DashboardContext, events: DashboardEvents): void {
  app.post("/api/builds/preview", async (request) => {
    const input = z.object({
      project_id: stableIdSchema, preview_id: stableIdSchema, event_id: z.string(),
    ...
    const preview = await createCompilePreview({
      workspaceRoot: context.workspaceRoot, projectId: projectId(input.project_id), previewId: resourceId(input.preview_id),
    ...
    events.publish({ type: "preview.changed", project_id: projectId(input.project_id), resource_kind: "preview", resource_id: preview.id, revision: preview.revision });
  ...
  app.get<{ Params: { projectId: string; previewId: string } }>("/api/builds/:projectId/preview/:previewId", async (request) => ({
    ok: true,
    data: await readCompilePreview(path.join(context.projectsRoot, projectId(request.params.projectId)), resourceId(request.params.previewId)),
  }));

  app.post("/api/builds/publish", async (request) => {
    const input = z.object({ project_id: stableIdSchema, preview_id: stableIdSchema, event_id: z.string() }).strict().parse(request.body);
    const result = await publishApprovedPreview({
      workspaceRoot: context.workspaceRoot,
      projectId: projectId(input.project_id),
      previewId: resourceId(input.preview_id),
      eventId: input.event_id,
      actor: "dashboard-user",
      occurredAt: new Date().toISOString(),
    });
    events.publish({ type: "build.published", project_id: projectId(input.project_id), resource_kind: "export", resource_id: input.preview_id, revision: result.preview.revision });
    return { ok: true, data: result };
  });

  app.get<{ Params: { projectId: string } }>("/api/builds/:projectId/exports", async (request) => {
    const root = path.join(context.exportsRoot, projectId(request.params.projectId));
    let names: string[] = [];
