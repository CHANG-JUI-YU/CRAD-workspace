...
import { evidenceRevision, normalizeFactClaim, sourceMatches, assertStrictFactQuality } from "./fact-policy.js";
...
/** Validate all typed claims before an operation is persisted or mutated. */
export function validateCurationClaims(state: ProjectState, claims: readonly FactClaim[]): void {
  claims.forEach((claim) => normalizeFactClaim(claim, state));
}
...
export function buildCurationCandidateBatch(state: ProjectState, claims: readonly FactClaim[], actor: string, curationRunId: string): CurationCandidateBatch {
  const normalizedClaims = claims.map((claim) => normalizeFactClaim(claim, state));
  const knownChunkSourceIds = new Set(state.knowledge_chunks.map((chunk) => chunk.source_id));
  const curationSources = state.sources.filter((source) => normalizedClaims.some((claim) => claim.evidence.some((evidence) => sourceMatches(source, evidence.source))));
  ...
  const candidates = normalizedClaims.map((claim) => {
    const fact = claimToFact(claim, state.sources, actor, curationRunId);
    const matcher = createEntityMatcher(state);
    const strict = state.interview.flow === "source_adaptation" || computeProjectProjection(state).intent.is_source_adaptation || matcher.entities.length > 0;
    assertStrictFactQuality(fact, { matcher, strictEntity: strict, strictCoverage: strict, strictQuality: strict });
    const evidenceRefs = claim.evidence.flatMap((evidence) => {
      ...
      return [{ source_id: source.id, source_revision_id: evidence.source_revision_id ?? source.revision, chunk_id: chunk.id, chunk_hash: chunk.hash, quote: evidence.quote, ...(evidence.locator === undefined ? {} : { locator: evidence.locator }) }];
    });
