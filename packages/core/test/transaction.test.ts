...
describe("file repository transaction and CAS", () => {
  ...
  it("refreshes a live lease so a transaction longer than thirty seconds is not stolen", async () => {
    ...
    try {
      ...
      const first = new FileProjectRepository(root, "demo", { lock: { lease_ms: 1_000, heartbeat_ms: 100, timeout_ms: 3_000 } });
      const second = new FileProjectRepository(root, "demo", { lock: { lease_ms: 1_000, heartbeat_ms: 100, timeout_ms: 400 } });
  ...
  it("uses the cross-instance lock when relocating a project", async () => {
    ...
    try {
      ...
      const initial = await bootstrap.read();
      const first = new FileProjectRepository(root, "demo", { lock: { lease_ms: 1_000, heartbeat_ms: 100, timeout_ms: 3_000 } });
      const second = new FileProjectRepository(root, "demo", { lock: { lease_ms: 1_000, heartbeat_ms: 100, timeout_ms: 1_200 } });
      const longCommit = first.transaction(initial.revision, async (state) => {
        await wait(2_500);
