...
const projectInputSchemas = {
  ...
  source_append_recuration: projectInput({
    ...
    file_path: z.string().min(1).describe("Explicit single local file path; mutually exclusive with bytes_base64"),
    bytes_base64: z.string().min(1).describe("Base64 content of a retrieved source; mutually exclusive with file_path"),
...
const descriptions: Record<keyof typeof allHandlers, string> = {
  ...
  source_append_recuration: "Atomically intake exactly one new source and return a Source Adaptation workflow from Facts Review to source processing in a single Director-authorized run.",
