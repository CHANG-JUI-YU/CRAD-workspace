...
const projectInputSchemas = {
  ...
  task_retry_begin: projectInput({
    ...workflowEvent,
    task_id: z.string().min(1).describe("Exact terminal failed workflow task ID"),
    run_id: z.string().regex(/^[a-z0-9]+(?:[._-][a-z0-9]+)*$/u).describe("Unique controlled retry run ID"),
    reason: z.string().min(1).describe("Auditable reason for retrying the invalid-output task with corrected submission"),
  }),
  ...
  source_append_recuration: projectInput({
    ...workflowEvent,
    run_id: z.string().regex(/^[a-z0-9]+(?:[._-][a-z0-9]+)*$/u).describe("Unique facts re-curation run ID"),
    reason: z.string().min(1).describe("Auditable quality reason for re-curating with the appended source"),
    source_id: z.string().min(1).describe("New source ID to intake before re-curation"),
    title: z.string().min(1).describe("Display title of the appended source"),
    file_path: z.string().min(1).optional().describe("Explicit single local file path; mutually exclusive with bytes_base64"),
    bytes_base64: z.string().min(1).optional().describe("Base64 content of a retrieved source; mutually exclusive with file_path"),
    requested_url: z.string().min(1).optional().describe("Requested URL for retrieved sources"),
    canonical_url: z.string().min(1).optional().describe("Canonical URL for retrieved sources"),
    fetched_at: z.string().min(1).optional().describe("ISO timestamp of the fetch for retrieved sources"),
    media_type: z.string().min(1).optional(),
    extension: z.string().min(1).optional(),
    language: z.string().min(1).optional(),
    recurate_source_ids: z.array(z.string().min(1)).min(1).optional().describe("Source IDs to re-curate; accepts bare IDs (e.g. novel) or prefixed IDs (source-novel); defaults to all sources when omitted"),
  }),
  ...
  fact_review_batch: projectInput({
    decisions: z.array(reviewDecisionSchema.extend({
      candidate_id: z.string().regex(/^candidate-occurrence-[a-f0-9]{64}$/u),
    })).min(1).max(50).describe("Review decisions applied atomically in one CAS commit; decision IDs and fact IDs must be unique within the batch"),
    expected_projection_revision: revisionSchema,
    expected_fact_revisions: z.record(z.string(), z.number().int().positive()).optional(),
    patches: z.record(z.string(), jsonObjectSchema).optional(),
  }),
  ...
  conflict_list: projectInput({
    limit: z.number().int().min(1).max(50).default(20).describe("Maximum conflicts per page"),
    cursor: z.string().min(1).describe("Opaque pagination cursor from the previous page").optional(),
  }),
...
const descriptions: Record<keyof typeof allHandlers, string> = {
  ...
  task_retry_begin: "Create one auditable successor for an attempts-exhausted task failed as invalid_output, preserving history and rewiring pending direct dependents.",
  ...
  source_append_recuration: "Atomically intake exactly one new source and return a Source Adaptation workflow from Facts Review to source processing in a single Director-authorized run. Omit recurate_source_ids to re-curate every source; pass the appended source ID to re-curate only the new source.",
  workflow_answer_interview: "Record one interview answer with workflow CAS.",
  workflow_approve_gate: "Relay a user gate approval only after OpenCode's explicit ask-permission confirmation. For the Content Gate, input_revisions must be exactly the workflow_status author-* artifacts (and approved plugin artifacts) with their exact revisions; the blueprint artifact is never part of the Content snapshot.",
  ...
  facts_review_status: "Read the bounded Source Adaptation candidate, decision, register, conflict, task, and job review status. Use overview.revisions.fact_register and overview.revisions.conflict_register as the exact Facts Gate input_revisions; fact_projection and conflict_projection are the semantic register revisions and are not accepted by the Facts Gate.",
  fact_query: "Query the projected fact register.",
  fact_review: "Apply a fact review decision with revision checks.",
  fact_review_batch: "Atomically apply up to 50 fact review decisions in one CAS commit and return a compact summary (reviewed, projection_revision, fact revisions, conflicts opened) instead of the full register snapshot.",
  ...
  conflict_list: "List open conflicts with exact member fact IDs (paginated), so the Director can resolve them with conflict_resolve without reading the full conflict register artifact.",
