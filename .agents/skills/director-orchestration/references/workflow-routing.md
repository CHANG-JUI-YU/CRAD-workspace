...
Blueprint 前的 interview 會先建立 roster；多角色由 Director 依每名角色暫稱逐一提出三個模式中立、以該角色核心設定為主的方向選項，單角色維持一次方向選擇。每個方向可附帶與 `{{user}}` 的關係起始影響，但不得把 Blueprint 方向縮成關係玩法。使用者的選擇、重新產生、混合或短句修改只保存為對應角色的 intake evidence；不得把某一名角色的修改套用到全 roster。Blueprint precheck 確認並物化 Blueprint artifact 後，才可建立 Zhuji 或 palette formal module tasks；Zhuji 的 `self_introduction` 是第七個正式模組，不是 interview 問題。

所有角色模式設定完成後，預設建立一個跨模式 Wardrobe Creator task。Wardrobe 讀取 Blueprint 與已完成的角色設定，輸出完整 Markdown 衣櫃；使用者明確要求跳過或延後時才不建立該 task。Wardrobe 與珠璣／調色盤使用相同的角色識別，不建立兩套模式格式。
...
多人角色卡訪談先由引擎把自然語言 roster 轉成穩定 subject，再詢問是否啟用 project-level relationships；啟用時可選完整 roster 或至少兩人的 participant subset，並將 exact IDs 寫入 Blueprint `relationships.character_ids`。Engine 只在 enabled 時建立 mode-neutral `create-relationships`，且等待每位 participant 的最後 required mode module；非 participant 不構成依賴。Director委派 `relationship-creator`，其只提交 structured `relationships` proposal，不可要求 thinking、roleplay或 `<team_CODE>` 編譯標記。Character Critic 的 exact inputs 必須包含 `relationship_module`。
